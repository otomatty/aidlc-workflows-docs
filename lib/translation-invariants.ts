...
// Relative upstream links are rewritten to blob URLs during translation, so
// blob URLs are excluded from the source/translation URL-set comparison.
const UPSTREAM_BLOB_PATTERN =
	/^https:\/\/github\.com\/awslabs\/aidlc-workflows\/blob\//;
...
export function extractRawUrls(text: string): string[] {
	const urls = (stripFrontmatter(text).match(RAW_URL_PATTERN) ?? [])
		.map((url) => url.replace(/[.,;:!?)}\]]+$/u, ""))
		.filter((url) => !UPSTREAM_BLOB_PATTERN.test(url));
...
export function collectTranslationInvariantErrors(
	...
	if (sourceUrls.join("\n") !== translationUrls.join("\n")) {
		errors.push(
			`raw URL set mismatch: ${translationPath} (source ${sourcePath})`,
		);
	}
	...
	if (!hasJapaneseProse(translationMarkdown)) {
		errors.push(
			`missing Japanese prose outside code fences: ${translationPath}`,
		);
	}
