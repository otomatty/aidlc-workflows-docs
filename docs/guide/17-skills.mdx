...
**ランナーが用意されるスコープは 4 つだけです。** 利用頻度が高いものに限られます。フレームワークが定義するスコープは全部で 9 個あり（[スコープ、深さ、テスト戦略](/guide/scopes-and-depth) を参照）、その他の `enterprise`、`poc`、`infra`、`refactor`、`workshop` は、常にオーケストレーター経由で到達できます。
