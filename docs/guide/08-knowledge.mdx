...
> **チーム固有のナレッジを注入するために Tier 1 ファイルを編集してはいけません。** `.claude/knowledge/` と `.claude/agents/*.md` は framework ファイルです。upgrade のたびに上書きされるため、変更は消えます。会社標準、アーキテクチャ上の好み、domain context を追加したいなら、**Tier 2**（下記）に追加してください。エージェントの振る舞いを制約したいなら、**ルール** を追加してください（[ルールと学習ループ](/guide/rules-and-the-learning-loop) を参照）。
...
完全なルール model、つまりファイル locations、5 層 chain、learning loop、admission-time conflict checks については [ルールと学習ループ](/guide/rules-and-the-learning-loop) を参照してください。
...
- [ルール](/guide/rules-and-the-learning-loop) は reference material ではなく behavioral constraints です。解決済み chain が最初に読み込まれ、適用可能なルールはすべてエージェントに届きます
...
methodology principle の適用の仕方を **制約したい** なら、Tier 1 ファイルを複製するのではなくルールを追加してください。[ルールと学習ループ](/guide/rules-and-the-learning-loop) を参照してください。
...
- [ルールと学習ループ](/guide/rules-and-the-learning-loop) — strict-additive ルール chain と、framework がワークフローを通じて新しいルールをどう学ぶか
- [はじめに](/guide/getting-started) — workspace shell と knowledge ディレクトリが現れる場所
