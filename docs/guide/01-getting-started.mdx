...
| フックの存在 | `settings.json` が配線しているすべてのフック（その `hooks` ブロックと `statusLine` コマンド、つまりフレームワークの 13 フック全体）が `.claude/hooks/` に存在すること。配線済みなのに欠けているフックは明確に失敗します。期待される一覧を `settings.json` から取るため、そこにフックを追加すれば自動的にチェック対象になります |
