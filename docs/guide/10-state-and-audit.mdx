...
| **移動** | 6 | `SCOPE_CHANGED`, `SCOPE_DETECTED`, `DEPTH_CHANGED`, `TEST_STRATEGY_CHANGED`, `RECOMPOSED`, `PLUGIN_SELECTION_CHANGED` |
| **対話** | 6 | `DECISION_RECORDED`, `GATE_APPROVED`, `GATE_REJECTED`, `QUESTION_ANSWERED`, `REVIEW_REQUESTED`, `REVIEW_COMPLETED` |
| **成果物** | 3 | `ARTIFACT_CREATED`, `ARTIFACT_UPDATED`（audit-logger フック）、`ARTIFACT_REUSED` |
| **サブエージェント** | 1 | `SUBAGENT_COMPLETED`（log-subagent フック） |
| **レビュアースコープ** | 1 | `REVIEWER_SCOPE_BLOCKED`（reviewer-scope フック） |
