...
schema、filename-to-scope table、resolver mechanics は Developer Reference の [Rule System](/reference/rule-system) に記載されています。
...
各ステージは、自分の outputs に対してどのセンサーを発火させるかを宣言します。独自のセンサーを追加することもできます。`.claude/sensors/` の下に manifest を著述し、その id を実行対象のステージに追加してください。ゲートで 1 つ確認すれば、learning loop がセンサーを install してくれることもあります。manifest format、per-stage matrix、authoring walkthrough は [Sensor System](/reference/sensor-system) にあります。自身の project へ追加する手順は [カスタマイズ](/guide/customization) を参照してください。
...
user としては、たいてい 1 回に 1 つの horizontal slice だけに触れます。ワークフローを走らせる、learning を捉える、team practices をカスタマイズする、起きたことを audit する、といった具合です。各 slice は、その下の planes に触れつつも、あなたにそれらを意識させません。完全な model、compile boundary、recovery property は [Plane Architecture](/reference/plane-architecture) にあり、そこで生じる telemetry 成果物は [Runtime Graph](/reference/runtime-graph) に記載されています。
...
- [ナレッジ](/guide/knowledge) — エージェントの行動を制約するのではなく、情報を与える 2 層 knowledge system
- [カスタマイズ](/guide/customization) — ルールやセンサーを追加し、loop を拡張し、ステージやエージェントを追加する
- [対話モード](/guide/interaction-modes) — ステージの中で corrections がどう起こるか
- [状態管理と監査証跡](/guide/state-and-audit) — learning loop の events がどう記録されるか
- [CLI コマンド](/guide/cli-commands) — doctor の rule-drift と paired-coverage advisory rows
- [Rule System](/reference/rule-system) · [Sensor System](/reference/sensor-system) · [Plane Architecture](/reference/plane-architecture) — schema と design の詳細リファレンス
