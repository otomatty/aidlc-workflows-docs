...
- **支援役によって、数を増やさずに協調できます。** "security-reviewer-agent" や "compliance-reviewer-agent" や "cost-reviewer-agent" を作る代わりに、aidlc-devsecops-agent と aidlc-compliance-agent が、他者主導のステージで支援エージェントとして参加します。インラインステージ（出荷されるグラフにあるすべての複数エージェントステージ）では、コンダクターは各支援エージェントを `Task` としてディスパッチするのではなく、自身のコンテキスト内でペルソナとして採用します。`Task` は `mode: subagent` のステージ専用です。いずれにせよ、すべての委譲を実行するのはコンダクターです。エージェントが互いを呼び出すことは決してありません。
...
もう 1 つ、両グループの外側に位置するエージェントがいます。`aidlc-composer-agent`、適応ワークフローのコンポーザーです。コンダクターはコンポーズ要求（`/aidlc compose`、コールドスタート時のコンポーズ提案、`--report`、または `--new-scope`）でこれをディスパッチします。このエージェントはタスクとワークスペース走査の結果を読み、EXECUTE/SKIP のステージグリッドを各 SKIP の根拠付きで提案します。そして、ゲートであなたが承認した *後にのみ*、コンポーズ済みスコープ（前方／報告モード）を著述するか、決定論的な `recompose` 動詞が適用する保留ステージの切り替え（実行中モード）を提案します。そのペルソナは意図的に「残す」方向へ偏らせてあります。存在理由を正当化し、欠落を吟味しますが、「速くするため」にステージを削ることは決してありません。[スコープ、深度、テスト戦略 - 適応コンポーザー](/guide/scopes-and-depth#the-adaptive-composer) を参照してください。
