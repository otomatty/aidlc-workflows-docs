...
`cp -R <src>/. <dst>/` の形式はツリーの**中身**をコピーします。`your-project/.kiro` が既に存在する場合（アップグレード）でも、存在しない場合（新規インストール）でも同じように動作します。単純な `cp -r dist/kiro-ide/.kiro your-project/.kiro` は、既存の `.kiro/` の内側に 2 つ目の `.kiro` を入れ子にしてしまい、IDE は新しいファイルを一切認識しません。
...
アダプターはフックの文脈を **環境変数 `USER_PROMPT`**（1.0 より前のチャネル）から読み取ります。コンテキストが stdin で届き `USER_PROMPT` が空になる IDE 1.x では、ペイロードに依存する 2 つのターゲット（`audit-and-sensors`、`log-subagent`）は発火するものの、可視のフックドロップを記録して何もしません。残りのフックはペイロードに依存せず、どちらの世代の IDE でも動作します。stdin コンテキストチャネルへの対応は、後続の拡張として予定されています。

1.0 より前のビルドでは、`USER_PROMPT` は JSON 文字列 `{ toolName, toolArgs, toolResult, toolSuccess }` です。IDE は `toolArgs` を空のままにするため、アダプターは `toolResult` のテキストから書き込まれたファイルパスを復元し、ペイロードを持たないフック（`runtime-compile`、`sync-statusline`）はツールのペイロードではなく監査証跡を基準に動かします。
...
| `aidlc-stop` | `Stop` | 転送ループの監査（勧告のみ。IDE では Stop トリガーはブロックできず、強制はコンダクター自身の Stop プロトコルに依存する） |
| `aidlc-block` | `PreToolUse` | 承認ゲートが開いたままで、その後に人間が操作していない間はツール呼び出しを強制ブロックする（human-presence フロア） |
| `aidlc-audit-logger` | `PostToolUse`（`fs_write\|str_replace\|fs_append`） | 成果物の作成 / 更新を記録し、続けて該当するセンサーを起動する（パスはツール結果から取得） |
| `aidlc-log-subagent` | `PostToolUse`（`invoke_sub_agent`） | 委譲先の識別情報とともに `SUBAGENT_COMPLETED` を記録する |
| `aidlc-runtime-compile` | `PostToolUse`（`execute_bash`） | 実行時グラフを再コンパイルする（監査末尾を条件に実行） |
| `aidlc-sync-statusline` | `PostToolUse`（`execute_bash`） | 監査内の最新 `STAGE_STARTED` から `Current Stage` を前進方向にのみ同期する（IDE は解析可能なタスクペイロードを渡さない） |
