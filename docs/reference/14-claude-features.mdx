...
| **常設ルール**（レイヤーチェーン） | `aidlc/spaces/<active-space>/memory/`（`.claude/rules/aidlc.md` の @インポート・スタブ経由） | `aidlc/spaces/<active-space>/memory/`（Kiro リソース・グロブ経由） | `aidlc/spaces/<active-space>/memory/`（Kiro リソース・グロブ経由） | `aidlc/spaces/<active-space>/memory/`（`AIDLC_RULES_DIR` 経由） | `aidlc/spaces/<active-space>/memory/`（`instructions` グロブ経由） |
