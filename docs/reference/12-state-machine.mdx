...
以下では **82 イベント**を 18 カテゴリに分類します（正規レジストリ
`audit-format.md` では同じ 82 イベントを 21 カテゴリに分けます。分類は表現上のもので、
...
| `REVIEW_COMPLETED` | `tools/aidlc-log.ts` | `READY` または `NOT-READY` のレビュアー判定を読み取ったときに出力し、宣言された出力パスとバイト列に対する `Artifact Fingerprint` を記録する。完了系のすべての状態遷移（`approve`、`advance`、`finalize`、`complete-workflow`）は、現在のワークフロー試行の、かつフィンガープリントがなお一致する受領記録を要求する。ユニット単位のステージは該当ユニットごとに 1 件を要求し、無効化をそのユニットに限定する。自律スウォームの確定は加えて、各設定済みユニットの Bolt 開始後の一致する受領記録と、該当する必須成果物のすべてがその Bolt ワークツリー内にファイルとして存在することを要求する（存在しない任意出力は有効なフィンガープリントエントリのまま）。 |

<a id="unit-lifecycle-inline-per-unit-construction-stages"></a>
...
| `UNIT_COMPLETED` | `tools/aidlc-state.ts` | `unit complete` — 確定前にそのユニットの必須成果物が通常ファイルであることを検証する。4 つのライフサイクルイベントはいずれも、厳密な境界イベント／タイムスタンプ／序数からなる `Run floor`（またはフェイルクローズのシャード横断曖昧性トークン）を伴い、受領記録モードは試行をまたいで有効なまま、同一シャード内の追記順が確定を制御し、順序の無い同一秒のシャード横断ライフサイクル同点は一時停止などの非終端状態を保持する。したがって古い・曖昧な・再オープンされたユニットは、再度完了するまでゲートをブロックする |
...
| `HUMAN_TURN` | `hooks/aidlc-record-human-turn.ts`（＋ハーネスごとのプロンプト送信アダプター） | 実際の人間プロンプトまたは回答済み質問ウィジェットごとに 1 件。承認 / インタビューゲートは、直前のゲート解決以降に 1 件を要求する |
...
| `REVIEW_FREEZE_BLOCKED` | `hooks/aidlc-review-freeze.ts` | ファイルツールまたはシェルによる `produces[]` への書き込みが、ゲート前に新鮮な READY レビュー受領記録を無効化するとして拒否された（§12a の終端受領記録の順序）。拒否ごとに 1 行 |
| `PLAN_APPROVAL_BLOCKED` | `hooks/aidlc-plan-approval-guard.ts` | コード生成の開発者エージェントディスパッチが、対象ユニットに空でない明示承認済みの `code-generation-plan.md` が無いとして拒否された（ステージ手順 2〜3 は手順 4 に先立つ必要がある）。拒否ごとに 1 行 |
