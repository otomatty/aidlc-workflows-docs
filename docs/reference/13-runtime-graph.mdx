...
[プレーンアーキテクチャ](/reference/plane-architecture)を、コンパイルを起動する遷移を持つ
ライフサイクルについては[状態機械](/reference/state-machine)を参照してください。
...
駆動します（[ステージ定義](/reference/stage-definition)の `produces_kinds` を参照）。
...
[プレーンアーキテクチャ](/reference/plane-architecture)に記載された根幹の原則に反します。
...
- **データプレーンをこのように構造化する理由** — `runtime-graph.json` を第 2 の
  ...
  データプレーンの分離です。[プレーンアーキテクチャ](/reference/plane-architecture)を参照してください。
- **コンパイルを起動するライフサイクル** — 監査出力がコンパイルフックを駆動する、
  ワークフロー / フェーズ / ステージの遷移です。[状態機械](/reference/state-machine)を参照してください。
- **このグラフの派生元となる監査ログ** — 74 イベントの分類と出力元レジストリです。
  [状態機械](/reference/state-machine)およびユーザーガイドの
  [状態と監査証跡](/guide/state-and-audit)を参照してください。
