...
この章が扱うのはマニフェストの**ファイル形式**です。センサーマニフェストに何が含まれるか、ステージがどうセンサーをインポートするか、出荷済み 4 つのマニフェストがどう設定されているかを説明します。ワークフロー中にセンサーがどう発火するかという利用者向け視点は、ユーザーガイドの [ルールと学習ループ](/guide/rules-and-the-learning-loop) を参照してください。
