...
<a id="pretooluse-aidlc-dispatch-rulests"></a>
...
**シェル経由の書き込み。** エンジンの無効化スキャンへデータを供給する audit-logger フックは `Write`/`Edit` の PostToolUse フックなので、シェルコマンドとして届くファイル変更はそのままでは不可視になり、変更後のバイト列に対して古い READY 受領記録を残してしまいます。そのため凍結処理は、`Bash` の実行前に出力リダイレクト先と一般的な変更コマンドのオペランドを抽出します。読み取り専用のシェル呼び出しは対象を生まないため通過します。
