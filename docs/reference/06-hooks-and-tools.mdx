...
17 本のうち 11 本は**非ブロッキング**です。6 本は**フロー変更型**です。`Stop` フックは転送ループを回し続け、ディスパッチルールフックは入力の書き換えをサポートするハーネスで正確なアクティブステージルールをサブエージェントブリーフへ付加し、プラン承認ガードは時期尚早なコード生成ディスパッチを拒否し、レビュースコープフックは兄弟ユニットへのレビュアーアクセスを拒否し、レビューフリーズフックは新鮮な READY レビュー受領記録をゲート前に無効化してしまう `produces[]` への書き込みを拒否し、状態遷移ガードは `aidlc-orchestrate.ts report` を迂回する直接のライフサイクル呼び出しを拒否します。
...
| `review-freeze.ts` | `PreToolUse` | プロジェクト全体（`settings.json`） | `Read\|Edit\|Write\|Glob\|Grep\|Bash`（変更を伴う呼び出しに自己フィルタ） | **フロー変更型。**§12a の終端受領記録の順序を決定論的に強制します。レビュアーを持ち、まだ完了していないステージが宣言する `produces[]` 成果物に対する `Write`/`Edit` またはシェルによる変更は、新鮮な READY レビュー受領記録がその成果物を覆っている間は拒否されます（終了コード 2 + リダイレクトされた標準エラー出力の理由）。シェル経由の書き込みは `Write`/`Edit` の監査フィードを通らず、そのままでは変更後のバイト列に対して古い受領記録を残してしまうため、実行前に検査します。エンジンと同一の受領記録スキャン（`aidlc-lib.ts` の `freshReviewReceipts`）を共有するため、ゲートでの却下・ジャンプ・ワークフロー再開始が記録されると凍結は自動的に解除され、NOT-READY 判定は決して凍結しません。各拒否は `REVIEW_FREEZE_BLOCKED` を出します。曖昧さがある場合はすべてフェイルオープンです。`AIDLC_DISABLE_REVIEW_FREEZE_HOOK=1` で強制を無効化できます |
...
| `rebuild-stage-graph.ts` | `PostToolUse` | プロジェクト全体（`settings.json`） | `Bash` | 遷移クラスの監査イベント出力後に `runtime-graph.json` を再コンパイルします |
...
3. **健全性ハートビート:** `.aidlc-hooks-health/rebuild-stage-graph.last` に書き込みます。
...
6. **人間待ち → 許可:** ディレクティブが保留中でも、コンダクターが正しく人間待ちしている（あるいは単に会話している）ならフックは停止を許可し、促しを連打せずに脱落を記録します。該当するのは 4 ケースです。現在のステージのチェックボックスが明示的に `[?]`（承認待ち）、`[R]`（改訂中）、未回答の `[Answer]:` タグを持つ `<slug>-questions.md` が存在する `[-]` 進行中（ステージ途中の確認質問が保留中）、あるいは終了ターン自体が会話だった場合です（ハーネストランスクリプトを読んで、「人間の直近プロンプトへワークフローエンジン呼び出しなしで答えた」ことを確認します）。最後の 2 ケースは自律構築では抑制されますが、ユニット主導のコード生成における必須の Plan Approval 質問だけは例外で、無関係な空欄質問は引き続き抑制されます。これは肯定確認のみです。その他の状態、チェックボックス行不在、未解決質問不在、トランスクリプトなし／人間プロンプトなし／応答ターン中にエンジン呼び出しがある場合、あるいは解析エラーの場合は、すべて下のブロックに流れます。詳しくは後述の「人間待ちの例外」を参照してください。
...
**目的:** §12a の終端受領記録の順序、つまり READY レビュー受領記録からゲートまでの書き込み凍結を決定論的に強制する
...
**判定。** 各書き込み対象について、状態ファイル上でまだ完了もスキップもされていないレビュアー付きステージすべてに対し、フックは次を確認します。パスが宣言された `produces[]` / `optional_produces[]` 成果物に一致するか（エンジン自身の接尾辞マッチャー `producesArtifactUnit`）、そして新鮮な READY 受領記録が現在それを覆っているか（`freshReviewReceipts` — エンジンの完了前提条件が読むのと**同じ**スキャンで、凍結ウィンドウと拒否ウィンドウが乖離しないよう `aidlc-lib.ts` で共有されています）。鮮度は監査上の時系列と、現在の成果物フィンガープリントの正確な一致の両方を要求します。ユニット単位ステージはレビュー済みユニットの成果物だけを凍結し、曖昧なユニット単位パスはいずれかのユニットが READY 受領記録を持つ場合に凍結します。NOT-READY 判定は決して凍結しません（修復ループが編集する必要があるため）。ゲートでの却下、ジャンプ、ワークフロー再開始、監査済みの書き込み、内容の不一致はいずれも受領記録を無効化します。ブロック時は `REVIEW_FREEZE_BLOCKED` 監査行（ツール、対象、ステージ、任意でユニット）を発行し、**終了コード 2 + リダイレクトされた標準エラー出力の理由**で、認められた経路（ゲートを提示してそこで提案を引用する、あるいはゲートで却下して成果物を再オープンする）を名指しします。

**シェル経由の書き込み。** エンジンの無効化スキャンへデータを供給する write-audit-log フックは `Write`/`Edit` の PostToolUse フックなので、シェルコマンドとして届くファイル変更はそのままでは不可視になり、変更後のバイト列に対して古い READY 受領記録を残してしまいます。そのため凍結処理は、`Bash` の実行前に出力リダイレクト先と一般的な変更コマンドのオペランドを抽出します。読み取り専用のシェル呼び出しは対象を生まないため通過します。

**身元判定なし。** レビュアースコープと違い、エージェントによるゲートはありません。`produces[]` への書き込みは、誰が行ったか（提案を適用するコンダクター、再ディスパッチされたリード、はぐれサブエージェント）に関係なく、新鮮な READY 受領記録を無効化します。
...
| `review-freeze.ts` | `REVIEW_FREEZE_BLOCKED` | `produces[]` への書き込みが、ゲート前に新鮮な READY レビュー受領記録を無効化するとして拒否されたとき（`PreToolUse`） |
