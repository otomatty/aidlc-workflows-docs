...
`aidlc/spaces/<space>/intents/<YYMMDD>-<label>/aidlc-state.md`（インテントの記録ディレクトリ）にある状態ファイルは、`.claude/knowledge/aidlc-shared/state-template.md` の契約に従ってエンジンが生成します。ステージの行は、テンプレートからではなく、コンパイル済みの `tools/data/stage-graph.json` と `scope-grid.json` に由来します。状態版 7 を使用し、以下を含みます。
