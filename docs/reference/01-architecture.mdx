...
このフレームワークは**1 回だけ著述し、ハーネスごとに生成**されます。現在は Claude Code、Kiro CLI、Codex CLI、GitHub Copilot、そしてそれを移植できるあらゆる CLI が対象です。
