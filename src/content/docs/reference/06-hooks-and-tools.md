---
title: フックとツール
description: フックシステムアーキテクチャ、12 本のフックスクリプト、監査イベント分類、CLI ツール設定、決定論的ユーティリティツールを文書化します。
sidebarOrder: 6
sourcePath: docs/reference/06-hooks-and-tools.md
sourceCommit: 3c76878775915b6dc510fa7e1ef0991ba510cd53
sourceHash: 750694922f476470c532a214a9d45c0a5a794a1cb278168ca67aca2027b05287
translationStatus: current
---

<a id="hooks-and-tools"></a>
# フックとツール

> 対象読者: ティア 2/3（チーム導入者、フレームワークコントリビューター）。

この章では、フックシステムアーキテクチャ、12 本すべてのフックスクリプト、監査イベントの分類、CLI ツール設定、そして決定論的ユーティリティツールを説明します。
...
この実装は `.claude/hooks/` 配下の 12 本のフックスクリプトを使います。12 本すべてが TypeScript（`bun` 経由で実行）です。12 本すべてが**プロジェクト全体**に対して登録されます。`settings.json` に登録されており（ステータスラインはトップレベルの `statusLine` キー、残り 11 本は `hooks` ブロック）、どのスキルが有効かに関係なく発火します。以前は分割されており（6 本は `aidlc/SKILL.md` のフロントマターにスキルスコープとして宣言され、残りがプロジェクト全体でした）、バージョン 0.6.0 でその 6 本も `settings.json` へ移されました。これにより、オーケストレーター、各パッケージ済みのスコープ／ステージランナー、そして手書きの顧客ランナーなど、どのエントリポイントもランナーごとの `hooks:` ブロックなしで決定論的な基幹機構を継承できます。これは安全です。なぜなら各フックが**自己ゲート**するからです。アクティブなワークフロー（`aidlc-state.md` またはアクティブなインテントの `audit/` シャード）が存在しないと早期終了するため、常時有効であっても AI-DLC 外では何もしません。

12 本のうち 10 本は**非ブロッキング**です。観測して終了コード 0 を返すだけで、制御フローを変えません。残る 2 本は**フロー変更型**であり、他のすべてのフックが守る助言的な `never-block` 契約とは別の、認可された明示的契約を持ちます。`Stop` フック（`aidlc-stop.ts`）は `{"decision":"block"}` を返して対話型の転送ループを継続させることができ（後述の「フロー変更型の `Stop` フック」を参照）、`PreToolUse` レビュースコープフック（`aidlc-reviewer-scope.ts`）は、ディスパッチ済みのユニット単位レビュアーが兄弟ユニットへ向かうツール呼び出しを、終了コード 2 とリダイレクトされた標準エラー出力の理由で拒否できます（後述の「フロー変更型の `PreToolUse` レビュースコープフック」を参照）。
...
12 本すべての TypeScript フックには、次の共通性があります。
...
- 成功時またはスキップ時には終了コード 0 を返す（`Stop` フックもブロック時は終了コード 0 です。ブロックは標準出力上の `{"decision":"block"}` JSON オブジェクトで示され、終了コードでは示されません。例外はレビュースコープフックで、こちらはハーネスの `PreToolUse` 拒否契約に従い、終了コード 2 + 標準エラー出力上の理由でブロックを通知します）
...
- ステージ 2.1（リバースエンジニアリング）-- 2 段階の委譲です（`aidlc-developer-agent` のコード走査で 1 回、その後 `aidlc-architect-agent` の合成で 1 回）
- ステージ 3.5（コード生成）-- `aidlc-developer-agent` サブエージェントです（作業ユニットごとに 1 回）
...
これはフレームワークにおける**最初のフロー変更型フック**です（下の `PreToolUse` レビュースコープフックが 2 つ目です）。他のすべてのフックは観測して終了コード 0 を返しますが、このフックはターンの終了を止めるために `{"decision":"block"}` を返すことがあります。ゲート付きかつ会話的な経路では、ループを保持しているのはコンダクター（LLM）です。なぜなら人間に質問できるのはコンダクターだけだからです。そのため、もしコンダクターがエンジンへ確認することを忘れるとワークフローがドリフトします。このフックは、その依存を LLM の注意深さから切り離します。ループはハーネスによって強制されます。
...
監査証跡（インテントの `audit/` シャード）は、`.claude/knowledge/aidlc-shared/audit-format.md` で定義された**71 イベント分類**を使います。すべてのイベントはツール所有またはフック所有であり、コンダクターが散文からイベントを発行することはもうありません。正準な発行者レジストリと監査優先の原子性規則については [状態機械](12-state-machine.md) を参照してください。以下の要約は真実の源泉ではなく相互参照です。
...
| **ワークフローライフサイクル** | 4 | `WORKFLOW_STARTED`, `WORKFLOW_COMPLETED`, `WORKFLOW_PARKED`, `WORKFLOW_UNPARKED` | `aidlc-utility.ts init`, `aidlc-state.ts complete-workflow`/`park`/`unpark` |
| **フェーズ** | 4 | `PHASE_STARTED`, `PHASE_COMPLETED`, `PHASE_VERIFIED`, `PHASE_SKIPPED` | `aidlc-utility.ts init`, `aidlc-state.ts advance` |
| **ステージ** | 6 | `STAGE_STARTED`, `STAGE_AWAITING_APPROVAL`, `STAGE_REVISING`, `STAGE_COMPLETED`, `STAGE_SKIPPED`, `STAGE_JUMPED` | `aidlc-state.ts`（ゲート開始／`approve`／`reject`／`skip`／`advance`）、`aidlc-jump.ts` |
...
| **対話** | 4 | `DECISION_RECORDED`, `GATE_APPROVED`, `GATE_REJECTED`, `QUESTION_ANSWERED` | `aidlc-log.ts`, `aidlc-state.ts` |
...
| **慣行** | 4 | `PRACTICES_DISCOVERED`, `PRACTICES_AFFIRMED`, `PRACTICES_OVERRIDE`, `PRACTICES_SECTION_EMPTY` | `aidlc-state.ts`（慣行昇格／慣行イベント） |
...
各ステージ実行は、正確に 2 つのイベントを生成しなければなりません。
- `STAGE_STARTED` -- コンダクターがステージを開始したときに記録
- `STAGE_COMPLETED` -- ステージが完了したときに記録（承認または自動進行）
...
| CLI ツール | その他すべてのイベント（ステージ／フェーズ／ワークフローのライフサイクル、ゲート、決定、ボルト、センサー、学習、復旧、…） | コンダクターが呼ぶツールサブコマンドが発行します。`aidlc-state.ts`、`aidlc-log.ts`、`aidlc-bolt.ts`、`aidlc-learnings.ts`、`aidlc-utility.ts` です。散文から手で追記してはいけません（`SKILL.md` の「散文から監査イベントを発行してはならない」を参照）。 |
...
| `persist --slug <stage-slug> --selections-json <path>` | 各確認済み学習を慣行として（既定スコープはプロジェクト）`aidlc/spaces/<space>/memory/project.md`／`memory/team.md` に日付付きエントリとして書き込みます。センサー束縛の学習では、プロジェクト階層マニフェストをスキャフォールドし、その識別子を発生元ステージの `sensors:` フロントマターへ追記します。両方の書き込みは 1 つの `withAuditLock` の中で行います | `RULE_LEARNED`, `SENSOR_PROPOSED` |
