...
sidebarOrder: 12
sourcePath: docs/reference/12-state-machine.md
sourceCommit: 3c76878775915b6dc510fa7e1ef0991ba510cd53
sourceHash: 0582b05f1d4276068c5c8ed5abd317e546fe84659d8bb0bd1271a3254d1aaf0e
...
すべてのフェーズへ `Pending` を設定し、対象範囲外のフェーズごとに（ステージ開始前に）
`PHASE_SKIPPED` を出力してから、現在のフェーズを `Active` に昇格させます。フェーズの
完了時には境界で `PHASE_COMPLETED` と `PHASE_VERIFIED` の両方を出力し、次のフェーズの
`PHASE_STARTED` を出力します。

| 遷移 | トリガー | エミッター |
|---|---|---|
| `Pending → Active`（最初のフェーズ） | `aidlc-utility intent-birth` | `tools/aidlc-utility.ts` |
| `Pending → Skipped` | `aidlc-utility intent-birth`（スコープ除外ごと） | `tools/aidlc-utility.ts` |
| `Active → Verified` | フェーズ境界での `aidlc-state advance` または `complete-workflow` | `tools/aidlc-state.ts` |
| `Pending → Active`（境界） | フェーズ境界での `aidlc-state advance`、または `aidlc-jump execute` | `tools/aidlc-state.ts`、`tools/aidlc-jump.ts` |
...
正当な `## Review` 追記を除外します。`codekb/` ステージは対象外です
（`produces` に `<slug>` サブディレクトリがありません）。空の台帳、ゲート開始の欠落、
...
以下では **71 イベント**を 17 カテゴリに分類します（正規レジストリ
`audit-format.md` では同じ 71 イベントを 19 カテゴリに分けます。分類は表現上のもので、
