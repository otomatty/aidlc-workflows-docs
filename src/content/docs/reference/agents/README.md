...
sidebarOrder: 0
sourcePath: docs/reference/agents/README.md
sourceCommit: 3c76878775915b6dc510fa7e1ef0991ba510cd53
sourceHash: 9d70f2928e9df0355221dfa9ad1603bd2d105999c608428dd9850e43c7423dc6
translationStatus: current
---

<a id="agent-reference"></a>
# エージェントリファレンス
...
## 13 エージェント一覧（11 のドメイン専門家 + 2 のレビュアー）
...
| 8 | [aidlc-developer-agent](developer-agent.md) | コード生成、ワークスペース検出、リバースエンジニアリング |
| 9 | [aidlc-quality-agent](quality-agent.md) | テスト戦略、テスト生成、性能検証 |
...
| 12 | aidlc-product-lead-agent | レビュー専用: 要件 / ユーザーストーリー / UX の品質ゲート（balanced tier） |
| 13 | aidlc-architecture-reviewer-agent | レビュー専用: 技術設計の健全性 / 実装可能性のゲート（balanced tier） |
...
すべてのエージェントは継承により Bash と WebSearch に *アクセス可能* ですが、この表が示しているのは各エージェントへの個別付与ではなく、方法論としてどのペルソナがそれらを使うことを **想定しているか** です。特定のペルソナを本当に制限したい場合は、任意の `tools:` 許可リストを追加してください（その場合、`mcp__<server>__<tool>` 識別子 も列挙しない限り継承された MCP は外れます）。この実装では、そのような制限は出荷していません。
...
| [aidlc-developer-agent](developer-agent.md) | reverse-engineering（コードスキャン）, code-generation | practices-discovery, functional-design, deployment-execution | judgment | Bash |
...
| エージェント | Bash | WebSearch | tier | 主担当ステージ数 | 支援ステージ数 | 総ステージ関与数 |
|-------|------|-----------|------|-------------|----------------|-------------------------|
...
| aidlc-developer-agent | あり | なし | judgment | 2 | 3 | 5 |
| aidlc-quality-agent | あり | なし | judgment | 2 | 2 | 4 |
...
- 14 体のエージェント全体では、9 体が `judgment` tier を持ち、5 体が一段下がります（2 体の `balanced` レビュアーと 3 体の `templated` プランナー）。一段下がるエージェントは、明示的なチェックリストに基づくレビュー、または強くテンプレート化された計画、CI/CD、ランブック作業を出力します。上のマトリクスは 11 体のドメイン専門家エージェントを対象にしています。
- aidlc-compliance-agent は純粋に助言役として動作します（アイデア創出、構築、運用 にまたがる 4 つの支援ステージで、主担当ステージはありません）。
...
| エージェント | 初期化（フェーズ 0） | アイデア創出（フェーズ 1） | 構想（フェーズ 2） | 構築（フェーズ 3） | 運用（フェーズ 4） |
|-------|--------------------------|---------------------|---------------------|------------------------|---------------------|
...
- [`SKILL.md`（コンダクター）](../../../dist/claude/.claude/skills/aidlc/SKILL.md) -- エンジンのディレクティブに従って動作する転送ループであり、人間が読めるステージグラフのミラーも備えています
